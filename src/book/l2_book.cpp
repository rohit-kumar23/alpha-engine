...
ApplyResult L2Book::apply(const md::MdEvent& event) {
    if (event.type == md::MdEventType::BookTicker) {
        if (event.bid_px > 0.0) {
    ...
    if (event.type != md::MdEventType::DepthUpdate) {
...
bool L2Book::seed_from_snapshot(const md::DepthSnapshot& snapshot) {
