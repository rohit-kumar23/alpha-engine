...
void OmsState::on_command_sent(const ordermgmt::OrderCommand& cmd) {
    ...
    if (cmd.type == ordermgmt::CommandType::New &&
    ...
    slot->status = (cmd.type == ordermgmt::CommandType::Replace) ? OrderStatus::PendingReplace : OrderStatus::PendingNew;
}

void OmsState::on_command_rejected(const ordermgmt::OrderCommand& cmd) {
    for (auto& order : orders_) {
        if (order.used && order.client_order_id == cmd.client_order_id) {
            if (order.status == OrderStatus::Completed || order.status == OrderStatus::Rejected) {
            ...
            if (order.status == OrderStatus::PendingCancel && cmd.type == ordermgmt::CommandType::Cancel) {
...
void OmsState::on_command_acked(const ordermgmt::OrderCommand& cmd) {
    for (auto& order : orders_) {
        if (order.used && order.client_order_id == cmd.client_order_id) {
            if (order.status == OrderStatus::Completed || order.status == OrderStatus::Rejected) {
