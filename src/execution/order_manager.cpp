...
std::size_t instrument_idx(md::Instrument instrument) {
    switch (instrument) {
        case md::Instrument::BtcUsdt:
            return 0;
        case md::Instrument::EthUsdt:
            return 1;
        case md::Instrument::SolUsdt:
            return 2;
        default:
            return 0;
    }
...
std::optional<OrderCommand> OrderManager::on_intent(md::Instrument instrument, const OrderIntent& intent, std::uint64_t ts_ns) {
...
void OrderManager::on_exec_report(const ExecReport& report) {
    for (auto& slot : slots_) {
        if (!slot.live || slot.client_order_id != report.client_order_id) {
            continue;
        }
        if (report.type == ExecEventType::Canceled || report.type == ExecEventType::Reject || report.terminal) {
            slot.live = false;
            slot.cancel_inflight = false;
            return;
        }
        if (report.type == ExecEventType::Ack) {
            slot.cancel_inflight = false;
        }
        return;
    }
