...
#include "hft/infra/exec_audit_log.hpp"
#include "hft/infra/spsc_ring.hpp"
#include "hft/md/binance_endpoints.hpp"
#include "hft/md/binance_parser.hpp"
#include "hft/md/binance_snapshot_client.hpp"
...
hft::md::BinanceExecutionMode read_binance_execution_mode() {
...
const char* binance_mode_label(hft::md::BinanceExecutionMode mode) {
    return mode == hft::md::BinanceExecutionMode::Live ? "live" : "demo";
}
...
void exec_audit_line(
    ...
    char buf[sizeof(hft::infra::ExecAuditRecord::data)];
    std::va_list ap;
    va_start(ap, fmt);
...
const char* instrument_name(hft::md::Instrument v) {
    using hft::md::Instrument;
    switch (v) {
...
std::size_t instrument_index(hft::md::Instrument v) {
    using hft::md::Instrument;
    switch (v) {
        case Instrument::BtcUsdt:
            return 0;
...
struct SnapshotRequest {
    hft::md::Instrument instrument {hft::md::Instrument::Unknown};
};

struct SnapshotResult {
    hft::md::Instrument instrument {hft::md::Instrument::Unknown};
    bool success {false};
    hft::md::DepthSnapshot snapshot {};
};
...
struct DepthEventBuffer {
    std::array<hft::md::MdEvent, Capacity> items {};
    std::size_t count {0};
    std::size_t drops {0};

    void push(const hft::md::MdEvent& event) {
        if (count < Capacity) {
            items[count++] = event;
            return;
        }
...
int main() {
    ...
    hft::infra::ExecAuditRing exec_audit_ring;
    hft::infra::ExecAuditLog exec_audit;
    ...
    hft::infra::ExecAuditRing md_health_ring;
    hft::infra::ExecAuditLog md_health_log;
    ...
    using hft::infra::SPSCRing;
    using hft::md::BinanceExecutionMode;
    using hft::md::BinanceFuturesEndpoints;
    using hft::md::BinanceParser;
    using hft::md::BinanceSnapshotClient;
    using hft::md::BinanceWsClient;
    using hft::md::futures_endpoints;
    using hft::md::Instrument;
    using hft::md::MdEvent;
    using hft::md::MdEventType;
    using hft::execution::CommandType;
    ...
    using hft::execution::OrderManager;
    using hft::execution::UserStreamParser;
    using hft::risk::RiskRejectReason;
    using hft::Side;
    ...
    hft::risk::PreTradeRisk risk(hft::risk::RiskConfig{
        risk_max_order_qty,
        risk_max_notional,
        risk_max_abs_pos,
        &g_kill_switch,
    ...
    std::array<hft::book::L2Book, 3> books;
    ...
    while (!g_stop.load(std::memory_order_relaxed)) {
        ...
        while (snapshot_res_queue.pop(snapshot_res)) {
            ...
            if (snapshot_res.success && books[idx].seed_from_snapshot(snapshot_res.snapshot)) {
                ...
                pending_depth[idx].replay([&](const MdEvent& e) {
                    ...
                    if (replay_result == hft::book::ApplyResult::OutOfSync) {
                        ++depth_out_of_sync;
                    }
                });
            } else {
        ...
        for (std::size_t md_n = 0; md_n < md_batch_max && queue.pop(event); ++md_n) {
            ...
            if (apply_result == hft::book::ApplyResult::OutOfSync) {
                ++depth_out_of_sync;
                pending_depth[idx].clear();
            }
